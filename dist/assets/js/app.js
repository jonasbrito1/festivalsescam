...
document.querySelectorAll('[data-toggle-menu]').forEach((button) => {
    button.addEventListener('click', () => {
        const sidebar = button.closest('.admin-sidebar');
        if (!sidebar) {
            return;
        }
...
if (offlineForm) {
    ...
    const enqueueCurrent = (reason = "offline") => {
        ...
        writeQueue(queue);
        saveDraft();
        setStatus("Conexao instavel. As notas foram guardadas neste dispositivo e serao enviadas automaticamente.", "pending");
    };
    ...
    const sendEntries = async (entries) => {
        ...
        try {
            ...
            if (error.name === "AbortError") {
                const timeoutError = new Error("O envio demorou demais. As notas foram guardadas para reenvio automatico.");
    ...
    const validateScores = () => {
        ...
        if (emptyField) {
            setStatus("Nao foi possivel salvar: preencha todas as notas antes de continuar.", "error");
            emptyField.focus();
            return false;
        }
    ...
    offlineForm.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            if (error.status && error.status < 500) {
                setStatus(error.message || "Nao foi possivel salvar as notas.", "error");
                return;
            }
